...
5. **Check Bone Configuration**
   - Click the "Configure…" button to check if the bones are correctly configured. Ensure there are no bones marked in red; if there are, corrections need to be made.

6. **Export as VRM**
   - In the Hierarchy window, select the model, then from the menu select `VRM > UniVRM > Export humanoid`.
   - Fill in the author information, then click "Export," choose the save location, and complete the export\[5].

Both methods effectively convert FBX files to VRM format. Depending on personal preference and tool availability, you can choose the method that suits you best for the conversion.
