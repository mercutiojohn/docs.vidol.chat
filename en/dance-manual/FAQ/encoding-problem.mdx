...
When downloading zip files from foreign websites, it is common to encounter issues where the filenames appear corrupted after extraction. This is usually caused by a mismatch between the system language and the language used when the compressed file was created.
...
<Note>
  This issue is particularly common when downloading MMD zip files from Japanese websites (such as Neconeco).
</Note>
...
After correctly setting it up, the extraction results should look like the image below:
